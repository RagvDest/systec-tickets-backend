...
describe('Estado', () => {
    ...
    it(`/GET buscarEstados de Ticket vacio`, async () => {
    ...
    it(`/POST crearEstado sin campo requerido`, async () => {
        let json = body;
        ...
        expect(response.text).toBe("Error registrar con los datos proporcionados");
  
        body.estado.e_detalle = e_detalle;

      });

      it(`/POST crearEstado con Ticket no existente`, async () => {
        let json = body;
      ...
      it(`/GET buscarEstados de Ticket`, async () => {
        const response = await request(app.getHttpServer())
          .get('/estado/all/'+id_ticket)
          .set('Authorization', 'Bearer '+process.env.TOKEN_TEST)
        expect(response.status).toBe(200);
      ...
      it(`/GET buscarEstados de Ticket invalido`, async () => {
        const response = await request(app.getHttpServer())
